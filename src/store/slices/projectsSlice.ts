...
export interface ProjectSettings {
  ...
  hasWebhook?: boolean
  defaultIntelligenceView?: string
  trackedBranches?: string[]
  branchProcessingMode?: 'default_only' | 'default_and_tracked'
}
...
export const updateProjectSettings = createAsyncThunk(
  ...
  async ({ id, data }: { id: string; data: { autoGenerateDocs?: boolean; githubAccessToken?: string | null; defaultIntelligenceView?: string; trackedBranches?: string[]; branchProcessingMode?: 'default_only' | 'default_and_tracked' } }, { rejectWithValue }) => {
    try {
...
export const inviteMember = createAsyncThunk(
  ...
  async ({ projectId, email, role }: { projectId: string; email: string; role?: 'owner' | 'admin' | 'member' }, { rejectWithValue }) => {
    try {
      const response = await projectsApi.inviteMember(projectId, email, role || 'member')
      return response.data
    } catch (error: any) {
