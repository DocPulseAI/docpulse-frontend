...
const cleanMarkdown = (raw: string): string => {
    ...
    // 1. Strip repetitive H1 titles at the very beginning (e.g. # DocPulseAI, # Overview, # Project Name)
    clean = clean.replace(/^#\s+(?:DocPulseAI|Overview|Project\s+\w+)\r?\n/i, '')
    clean = clean.replace(/_Generated by DocPulseAI_.*\r?\n/gi, '')

    // 2. Strip metadata blocks at the top (e.g., lists of commit, branch, generated time)
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const isMetaPattern = /^(?:commit|branch|version|generated|timestamp|project)\s*:\s*.+/i.test(line) ||
            /^(?:-\s+|\*\s+)(?:commit|branch|version|generated|timestamp|project)\s*:\s*.+/i.test(line)

        if (isMetadataBlock) {
            if (isMetaPattern || line === '' || /^\|?\s*(?:commit|branch|version|generated)\s*\|/i.test(line) || /^\|?[-:\s|]+\|?$/.test(line)) {
                continue
            } else {
    ...
    // 3. Strip empty sections (e.g., ## Risks followed directly by another header)
    clean = clean.replace(/\n\s*\n/g, '\n\n')
    clean = clean.replace(/^(#{2,6}\s+.*)\n+(?=#{2,6}\s+)/gm, '')
    clean = clean.replace(/^(#{2,6}\s+.*)\n+\s*(?:No\s+\w+\s+(?:found|recorded|available|detected|issues)|None)\.?\s*(?=\n|#|$)/gmi, '')

    // 4. Strip zero-value metric rows or list items
    clean = clean.replace(/^(?:-\s+|\*\s+)?(?:errors|warnings|drift\s+score|breaking\s+changes|issues|risks|critical\s+issues)\s*:\s*0\s*$/gmi, '')
