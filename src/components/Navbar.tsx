    ...
    <nav className="navbar">
      <Link to="/" className="navbar-brand" aria-label="Go to landing page">
        ...
        <h1 className="navbar-wordmark">
          DocPulse<span style={{ color: 'var(--accent-primary)' }}>AI</span>
        </h1>
      </Link>
      ...
      <div className={`navbar-links ${mobileOpen ? 'is-open' : ''}`}>
        ...
        <div className="navbar-links-meta">
          {/* Cmd+K hint */}
          <button className="navbar-cmd-btn" onClick={() => document.dispatchEvent(new KeyboardEvent('keydown', { key: 'k', metaKey: true }))}>
            <Search size={13} />
            <span>Search</span>
